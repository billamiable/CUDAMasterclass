...
int main()
{
	...
	//allocate pinned memory for host size pointers
	...
	// sum two arrays into one array
	initialize(h_a, INIT_ONE_TO_TEN);
	initialize(h_b, INIT_ONE_TO_TEN);

	//summation in CPU for comparison
	...
	// this value is changed, previously, it is (size/block.x + 1)
	...
	for (int  i = 0; i < NUM_STREAMS; i++)
	{
		...
		// only provide address for starting element in the array is fine
	...
	// destroy cuda streams in the end
	for (int i = 0; i < NUM_STREAMS; i++)
	{
		cudaStreamDestroy(streams[i]);
	}

	// seems no use of cudaStreamSynchronize
	// but cudaDeviceSynchronize is alread fine, because no dependency presented here
